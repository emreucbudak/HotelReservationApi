...
namespace HotelReservationApi.Application.Features.CQRS.Rooms.Queries.GetAllForReservation
{
    public class GetAllRoomsForReservationQueriesHandler : IRequestHandler<GetAllRoomsForReservationQueriesRequest, List<GetAllRoomsForReservationQueriesResponse>>
    {
        ...
        {
            var rooms = await unitOfWork.readRepository<Domain.Entities.Rooms>().GetAllWithPaging(enableTracking:false,predicate: x=> x.RoomTypes.Hotels.HotelAdress.City.Name == request.City && x.RoomTypes.Hotels.HotelAdress.District.Name == request.District && !x.reservationRooms.Any(z =>    z.Reservation.StartDate < request.leftDate && z.Reservation.EndDate > request.entryDate),page:request.Page,size:request.PageSize,includable:x=> x.Include(x=> x.RoomTypes));
